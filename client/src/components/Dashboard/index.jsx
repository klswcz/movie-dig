        ...
        <div className="h-screen-minus-navbar pb-14 px-4">
            ...
            <div className="grid xl:grid-cols-8 lg:grid-cols-6 md:grid-cols-4 grid-cols-2">
                {trendingMovies.map((movie, index) => {
                    return (
                        <MovieCard title={movie.title ?? movie.name} voteAverage={movie.vote_average} posterPath={movie.poster_path} userRating={movie.user_rating} key={index} movieId={movie.id}/>
                    )
                })}
            </div>
        </div>
