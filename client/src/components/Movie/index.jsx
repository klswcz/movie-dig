...
function Movie(props) {
    ...
    const addWishlistItem = () => {
        setIsWishlistRequestProcessed(true)
        createWishlistItemService({ movie_id: movieId }).then(() => {
    ...
    const deleteWishlistItem = () => {
        setIsWishlistRequestProcessed(true)
        destroyWishlistItemService({ movie_id: movieId }).then(() => {
                    ...
                    <div className="block sm:hidden mb-2">
                        <p className="">
                            ...
                            {movie.credits.crew.find((crewMember) => crewMember.job === "Director")
                                ? movie.credits.crew.find((crewMember) => crewMember.job === "Director").name
                                : "N/A"}
                        </p>
                        <p className="">
                            <span className="font-bold">Main writer</span>:{" "}
                            {movie.credits.crew.find((crewMember) => crewMember.job === "Screenplay")
                                ? movie.credits.crew.find((crewMember) => crewMember.job === "Screenplay").name
                                : "N/A"}
                        </p>
                        <p className="">
                            <span className="font-bold">Producer</span>:{" "}
                            {movie.credits.crew.find((crewMember) => crewMember.job === "Producer")
                                ? movie.credits.crew.find((crewMember) => crewMember.job === "Producer").name
                                : "N/A"}
                        </p>
                    </div>
                    <div className="inline-block sm:hidden mb-3">
                        {movie.genres &&
                            movie.genres.map((genre, index) => {
                                return (
                                    <p
                                        className="inline-block mr-3 px-3 py-1 mb-2 text-white rounded-md bg-green-500"
                                        key={index}
                                    >
                                        {genre.name}
                                    </p>
                    ...
                    <div className="flex flex-wrap">
                        <div className="w-full sm:w-1/4">
                            {movie.poster_path ? (
                                ...
                                <img
                                    src={"https://dummyimage.com/300x450/cccccc/ffffff&text=No+poster"}
                                    alt=""
                            ...
                            <div className="">
                                <p className="my-2 font-extrabold text-gray-900">Your rating</p>
                            ...
                            <button
                                onClick={isWishlistItem ? deleteWishlistItem : addWishlistItem}
                        ...
                        <div className="w-full sm:w-3/4 sm:pl-7">
                            ...
                            <h2 className="text-2xl font-extrabold text-gray-900 mt-5 mb-1 hidden sm:block">
                                Description
                            </h2>
                                        ...
                                        return (
                                            <div className="text-center flex items-center flex-col mr-10" key={index}>
                                                {actor.profile_path ? (
                                                    ...
                                                    <img
                                                        src={
                                                            "https://dummyimage.com/300x450/cccccc/ffffff&text=No+image"
                                                        }
                                                        alt=""
                                                        className="rounded-md align-top sm:align-middle inline-block mr-3"
                                                    />
                            ...
                            <h2 className="text-2xl mt-5 font-extrabold text-gray-900">
                                People who enjoyed this movie also liked
                            </h2>
                            <div>
                                <HorizontalScroll>
